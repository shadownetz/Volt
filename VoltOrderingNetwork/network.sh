...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating VoltLaundry Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-voltlaundry.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating VoltLogistics Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-voltlogistics.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  infoln "Generating CCP files for VoltLaundry and VoltLogistics"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker volume rm docker_orderer.vltenterprise.com docker_peer0.voltlaundry.vltenterprise.com docker_peer1.voltlaundry.vltenterprise.com docker_peer2.voltlaundry.vltenterprise.com docker_peer0.voltlogistics.vltenterprise.com docker_peer1.voltlogistics.vltenterprise.com docker_peer2.voltlogistics.vltenterprise.com
...
CLI_DELAY=3
# channel name defaults to "orderingchannel1"
