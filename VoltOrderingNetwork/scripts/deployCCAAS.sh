...
CHANNEL_NAME=${1:-"orderingchannel1"}
CC_NAME=${2}
CC_SRC_PATH=${3}
...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    docker run --rm -d --name peer0voltlaundry_${CC_NAME}_ccaas  \
                  --network volt_ordering_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    docker run  --rm -d --name peer0voltlogistics_${CC_NAME}_ccaas \
                  --network volt_ordering_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    docker run --rm -d --name peer0voltlaundry_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
infoln "Installing chaincode on peer0.voltlaundry..."
installChaincode "voltlaundry"
infoln "Install chaincode on peer0.voltlogistics..."
installChaincode "voltlogistics"

## query whether the chaincode is installed
queryInstalled "voltlaundry"

## approve the definition for org1
approveForMyOrg "voltlogistics"

## check whether the chaincode definition is ready to be committed
## expect org1 to have approved and org2 not to
checkCommitReadiness "voltlaundry" "\"VoltLaundryMSP\": true" "\"VoltLogisticsMSP\": false"
checkCommitReadiness "voltlogistics" "\"VoltLaundryMSP\": true" "\"VoltLogisticsMSP\": false"
...
checkCommitReadiness "voltlaundry" "\"VoltLaundryMSP\": true" "\"VoltLogisticsMSP\": true"
checkCommitReadiness "voltlogistics" "\"VoltLaundryMSP\": true" "\"VoltLogisticsMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition "voltlaundry" "voltlogistics"

## query on both orgs to see that the definition committed successfully
queryCommitted "voltlaundry"
queryCommitted "voltlogistics"
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit "voltlaundry" "voltlogistics"
fi
